"""Run the two-iteration Linux round tail with a helper that is just RET."""
...
PROGRAM_WORDS[HELPER_BASE] = 0x00008067
...
def _execute_program(initial_regs: list[int], initial_mem: dict[int, int]):
    ...
    while True:
        ...
        if opcode == 0x03:
            ...
            imm = STITCH._sign_extend(instr >> 20, 12)
            assert funct3 == 0x0, f"Unexpected I-type op: {instr:08x}"
        ...
        steps += 1
        if steps > max_steps:
            raise AssertionError("RET-helper round loop did not terminate in model")
...
@cocotb.test()
async def test_round_loop_two_iter_ret_helper_matches_model(dut):
    ...
    for _ in range(1200):
        ...
        trace.append(
            (
                ...
                int(dut.cpu_inst.id_ex_inst0_pc_out.value) & 0xFFFF_FFFF,
                int(dut.cpu_inst.id_ex_inst0_instr_id_out.value) & 0x7F,
                int(dut.cpu_inst.id_ex_inst0_instr_valid_out.value) & 0x1,
                int(dut.cpu_inst.ex_mem_inst0_pc_out.value) & 0xFFFF_FFFF,
                int(dut.cpu_inst.ex_mem_inst0_instr_id_out.value) & 0x7F,
                int(dut.cpu_inst.ex_mem_inst0_rd_valid_out.value) & 0x1,
                int(dut.cpu_inst.mem_wb_inst0_pc_out.value) & 0xFFFF_FFFF,
                int(dut.cpu_inst.mem_wb_inst0_instr_id_out.value) & 0x7F,
                int(dut.cpu_inst.mem_wb_inst0_rd_valid_out.value) & 0x1,
                int(dut.cpu_inst.wb_inst0_rd_addr_out.value) & 0x1F,
                int(dut.cpu_inst.wb_inst0_rd_value_out.value) & 0xFFFF_FFFF,
                int(dut.cpu_inst.module_mem_wr_en.value) & 0x1,
                int(dut.cpu_inst.module_write_addr.value) & 0xFFFF_FFFF,
                int(dut.cpu_inst.module_wr_data_out.value) & 0xFFFF_FFFF,
            )
        )
    ...
    assert done, "RET-helper round loop test never completed"
    ...
    if mismatches:
        ...
        raise AssertionError(
            f"RET-helper round-loop mismatch: {preview}; "
            f"s1=0x{actual_regs[9]:08x} s2=0x{actual_regs[18]:08x} "
            f"s3=0x{actual_regs[19]:08x} s6=0x{actual_regs[22]:08x} "
            f"a0=0x{actual_regs[10]:08x} a2=0x{actual_regs[12]:08x} "
            f"a3=0x{actual_regs[13]:08x} a4=0x{actual_regs[14]:08x} "
            f"a5=0x{actual_regs[15]:08x}; "
            f"trace={' '.join(f'pc=0x{pc:08x}/i=0x{insn:08x}/id=0x{idpc:08x}:{idinsn:02x}:v{idv}/exm=0x{expc:08x}:{exinsn:02x}:v{exv}/wb=0x{wbpc:08x}:{wbinsn:02x}:v{wbv}/wbr=x{wbrd}:0x{wbrv:08x}/memw={mw}:0x{mwa:08x}:0x{mwd:08x}' for pc, insn, idpc, idinsn, idv, expc, exinsn, exv, wbpc, wbinsn, wbv, wbrd, wbrv, mw, mwa, mwd in trace[-24:])}"
        )
    ...
    assert actual_regs == expected_regs, (
        f"Register mismatch in RET-helper round loop: "
...
def runCocotbTests():
    ...
    sim_build = Path.cwd() / "sim_build" / "sim_build_round_loop_two_iter_ret_helper"
    if sim_build.exists():
    ...
    run(
        ...
        toplevel="top",
        module="test_round_loop_two_iter_ret_helper",
        testcase="test_round_loop_two_iter_ret_helper_matches_model",
