"""Checkpoint the real Linux caller/helper loop after a chosen round count."""
...
TARGET_ROUND = int(os.environ.get("LINUX_CALLER_CHECK_ROUND", "1"))
ROUND_PC = 0x8000_568C
...
def _model_state_after_rounds(initial_regs: list[int], initial_mem: dict[int, int], round_count: int):
    regs = initial_regs[:]
    ...
    while rounds_seen < round_count:
        ...
        steps += 1
        if steps > max_steps:
            raise AssertionError("Model did not reach target round")

    return regs, mem


@cocotb.test()
async def test_linux_caller_round_checkpoint_matches_model(dut):
    ...
    _, expected_mem = _model_state_after_rounds(init_regs, init_mem, TARGET_ROUND)
    ...
    for _ in range(90000):
        ...
        if (int(dut.pc_debug.value) & 0xFFFF_FFFF) == ROUND_PC:
            rounds_seen += 1
            if rounds_seen == TARGET_ROUND:
                reached = True
                break

    assert reached, f"RTL never reached round checkpoint {TARGET_ROUND}"
    ...
    preview = ", ".join(
        ...
        for addr, expected, actual in mismatches
    ) or "none"

    assert not mismatches, f"Round {TARGET_ROUND} mismatch: {preview}"
...
def runCocotbTests():
    ...
    sim_build = Path.cwd() / "sim_build" / f"sim_build_linux_caller_round_{TARGET_ROUND}"
    if sim_build.exists():
    ...
    run(
        ...
        toplevel="top",
        module="test_linux_caller_round_checkpoint",
        testcase="test_linux_caller_round_checkpoint_matches_model",
