"""Run the final Linux caller transition window into memset.

This starts from an exact architectural checkpoint shortly before the final
copy/clear tail and executes through the far memset call and return sentinel.
It narrows whether the stitched failure needs the whole caller history or just
the short transition window into the tail.
"""
...
TRANSITION_START = int(os.environ.get("LINUX_TRANSITION_START", "0x800056f8"), 16)
TAIL_END = 0x8000_5770
TEST_SEED = int(os.environ.get("LINUX_TRANSITION_SEED", "0"))
...
PROGRAM_WORDS = {INSTR_MEM_BASE: _encode_jal(0, INSTR_MEM_BASE, TRANSITION_START)}
for addr, word in HELPERS.PROGRAM_WORDS.items():
    if TRANSITION_START <= addr < TAIL_END:
...
def _transition_entry_state(seed: int):
    init_regs, init_mem, _ = HELPERS._seed_state(seed)
    regs, mem = PHASES._model_state_at_pc(init_regs, init_mem, TRANSITION_START, 1)
    regs = regs[:]
    regs[0] = 0
    return regs, mem
...
def _execute_program(initial_regs: list[int], initial_mem: dict[int, int]):
    ...
    while True:
        ...
        steps += 1
        if steps > max_steps:
            raise AssertionError("Tail transition test did not terminate in model")
...
@cocotb.test()
async def test_linux_tail_transition_matches_model(dut):
    ...
    init_regs, init_mem = _transition_entry_state(TEST_SEED)
    ...
    assert done, "Tail transition test never completed"
    ...
    assert not mismatches, (
        f"Tail transition mismatch on seed {TEST_SEED} start=0x{TRANSITION_START:08x}: {preview}"
    )
    ...
    assert actual_regs == expected_regs, (
        f"Register mismatch in tail transition test on seed {TEST_SEED} "
        f"start=0x{TRANSITION_START:08x}: "
...
def runCocotbTests():
    ...
    sim_build = Path.cwd() / "sim_build" / "sim_build_linux_tail_transition"
    if sim_build.exists():
    ...
    run(
        ...
        toplevel="top",
        module="test_linux_tail_transition",
        testcase="test_linux_tail_transition_matches_model",
