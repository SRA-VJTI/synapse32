"""Verify store data forwards across a one-instruction gap from WB stage."""
...
PROGRAM_WORDS = [
    ...
    0x00000E93,  # li t4, 0        # unrelated gap instruction
...
@cocotb.test()
async def test_store_data_forwards_from_wb_stage_to_store(dut):
    ...
    assert done, "WB-gap store-data forwarding test never completed"

    actual = int(dut.unified_mem_inst.instr_ram[_phys_word_index(DATA_MEM_BASE)].value) & 0xFFFF_FFFF
    assert actual == 0x00000071, (
        f"Store consumed stale rs2 value across gap: expected=0x00000071 actual=0x{actual:08x}"
    )
...
def runCocotbTests():
    ...
    sim_build = Path.cwd() / "sim_build" / "sim_build_store_data_forwarding_wb_gap"
    if sim_build.exists():
    ...
    run(
        ...
        toplevel="top",
        module="test_store_data_forwarding_wb_gap",
        testcase="test_store_data_forwards_from_wb_stage_to_store",
