"""Verify a loaded register survives xor+gap before being stored back."""
...
async def _load_program(dut) -> None:
    ...
    dut.unified_mem_inst.instr_ram[_phys_word_index(DATA_MEM_BASE)].value = 0x00000070


@cocotb.test()
async def test_loaded_xor_value_stores_after_gap(dut):
    ...
    assert done, "Load-xor-store-gap test never completed"

    actual = int(dut.unified_mem_inst.instr_ram[_phys_word_index(DATA_MEM_BASE)].value) & 0xFFFF_FFFF
    assert actual == 0x00000071, (
        f"Store lost xor result derived from load: expected=0x00000071 actual=0x{actual:08x}"
    )
...
def runCocotbTests():
    ...
    sim_build = Path.cwd() / "sim_build" / "sim_build_load_xor_store_gap"
    if sim_build.exists():
    ...
    run(
        ...
        toplevel="top",
        module="test_load_xor_store_gap",
        testcase="test_loaded_xor_value_stores_after_gap",
